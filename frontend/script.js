...
async function loadReportsSummary() {
  ...
  const usersEl = document.getElementById("report-users");
  const symbolsEl = document.getElementById("report-symbols");
  ...
  try {
    ...
    if (!res.ok) {
      if (msg) {
        msg.textContent = data.message || "Failed to load reports.";
        msg.className = "message error";
      }
      return;
    }
    ...
    if (symbolsEl) symbolsEl.textContent = data.popular_symbols.join(", ");
  } catch (e) {
    if (msg) {
      msg.textContent = "Network error while loading reports.";
      msg.className = "message error";
    }
  }
}
