...
# MySQL config – EDIT for your MySQL setup
app.config["DB_HOST"] = "localhost"
app.config["DB_USER"] = "root"
app.config["DB_PASSWORD"] = ""  # your MySQL password
app.config["DB_NAME"] = "stock_prediction"

# Adjust origin to where your frontend runs (e.g. Live Server)
...
def list_users():
  conn = get_db()
  cur = conn.cursor(dictionary=True)
  cur.execute("SELECT id, name, email, phone, role FROM users ORDER BY id")
  users = cur.fetchall()
  cur.close()
  conn.close()
...
def reports_summary():
  """
  Simple placeholder 'reports' endpoint.
  """
  conn = get_db()
  cur = conn.cursor()
